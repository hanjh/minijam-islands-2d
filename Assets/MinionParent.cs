...
public class MinionParent : MonoBehaviour
{
    ...
    public void handleMouseDown() {
        ...
        if (hit.transform.gameObject.layer == LayerMask.NameToLayer("Player")) {
            ...
            offset = startTransform.position - Camera.main.ScreenToWorldPoint(Input.mousePosition);
        }
       if (isActiveArrow == false) {
            createArrow(gridPosition);
        } else {
        }
    }
    ...
    public void handleMouseUp() {
        ...
        {
            GameObject newMinion = Instantiate(playerMinion, new Vector3(startTransform.position.x, startTransform.position.y, 0), 
                Quaternion.identity);
    ...
    public void createArrow(Vector2Int gridPosition) {
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0)) {
            handleMouseDown();
        } else if (Input.GetMouseButtonUp(0) && startTransform != null) {
            handleMouseUp();
        }
